...
# Loom Workflow — Windsurf

> **Setup:** If Loom tools are not available, run `loom install` in the project
> root. This writes the MCP config to `~/.codeium/windsurf/mcp_config.json`.
> Restart Windsurf after running.

## Session start

```
start_session(agent_id="windsurf")     # store session_id for next time
```

If you have a previous session_id:
```
get_delta(previous_session_id="<id>")  # only what changed since last session
```
...
`summary_stale: true` → source changed → re-read + update summary.
