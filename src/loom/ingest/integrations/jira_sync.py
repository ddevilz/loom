    ...
    *,
    ...
    for issue in issues:
        ...
        if status == _WONT_FIX_STATUS:
            await graph.query(
                "MATCH ()-[r:LOOM_IMPLEMENTS]->(t {id: $ticket_id}) SET r.stale = true, r.stale_reason = 'ticket_wont_fix'",
                {"ticket_id": ticket_id},
            )
            continue
        ...
        if status in _REOPENED_STATUSES:
            await graph.query(
                "MATCH ()-[r:LOOM_IMPLEMENTS]->(t {id: $ticket_id}) SET r.needs_review = true, r.review_reason = 'ticket_reopened'",
                {"ticket_id": ticket_id},
            )

        if linker is not None:
            rows = await graph.query(
                "MATCH (f)-[:LOOM_IMPLEMENTS]->(t {id: $ticket_id}) RETURN f.id AS id, f.kind AS kind, f.name AS name, f.summary AS summary, f.path AS path, f.metadata AS metadata, f.embedding AS embedding",
                {"ticket_id": ticket_id},
            )
            code_nodes = [
                Node(
