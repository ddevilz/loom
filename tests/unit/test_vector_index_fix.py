...
async def test_search_uses_vector_index_when_available() -> None:
    ...
    assert not graph.fallback_query_attempted, (
        "Fallback should NOT be used when vector index works"
    )
    assert len(results) > 0
    assert results[0].matched_via == "vector", (
        "Results should be marked as coming from vector index"
    )
...
async def test_search_falls_back_when_vector_index_fails() -> None:
    ...
    assert graph.fallback_query_attempted, (
        "Fallback should be used when vector index fails"
    )
    assert len(results) > 0
    assert results[0].matched_via == "vector_fallback", (
        "Results should be marked as coming from fallback"
    )
...
def test_schema_init_creates_valid_vector_index_ddl() -> None:
    ...
    vector_index_queries = [
        q for q in gw.executed_queries if "CREATE VECTOR INDEX" in q
    ]
    ...
    assert "FOR (n:Node) ON (n.embedding)" in vector_index_ddl, (
        "Index should target Node.embedding with parenthesised attribute syntax"
    )
