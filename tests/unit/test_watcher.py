...
class _FakeGraph:
    ...
    async def query(self, cypher: str, params: dict[str, Any] | None = None):
        ...
        if q == "MATCH (n {path: $path}) RETURN n.id AS id":
            ...
            return [{"id": node_id} for node_id in self.ids_by_path.get(params["path"], [])]
...
async def test_watch_repo_reindexes_on_file_change(tmp_path: Path) -> None:
    ...
    assert any("MATCH (n {path: $path})-[r:LOOM_IMPLEMENTS]->()" in q for q in loom_queries)
    assert any("MATCH ()-[r:LOOM_IMPLEMENTS]->(n {path: $path})" in q for q in loom_queries)


@pytest.mark.asyncio
async def test_watch_repo_flags_modified_file_edges_before_reindex(tmp_path: Path) -> None:
    call_order: list[str] = []
...
async def test_watch_repo_deletes_nodes_on_removed_file(tmp_path: Path) -> None:
    ...
    delete_index = next(i for i, q in enumerate(graph.queries) if "DETACH DELETE" in q[0])
    assert len(loom_queries) == 2
    assert all(index < delete_index for index in loom_queries)


@pytest.mark.asyncio
async def test_watch_repo_preserves_deleted_nodes_with_human_edges(tmp_path: Path) -> None:
    async def fake_events(_path: str):
        yield {(Change.deleted, str(tmp_path / "gone.py"))}
    ...
    delete_by_ids_queries = [params for q, params in graph.queries if q.strip() == "UNWIND $ids AS id MATCH (n {id: id}) DETACH DELETE n"]
    invalidation_queries = [params for q, params in graph.queries if q.strip().startswith("MATCH (a {path: $path})-[r]->()") or q.strip() == "MATCH ()-[r]->(a {path: $path})\nWHERE r.origin IS NULL OR r.origin <> 'human'\nDELETE r" or q.strip() == "MATCH ()-[r]->(a {path: $path})\nWHERE r.origin = 'human'\nSET r.stale = true,\n    r.stale_reason = 'source_changed'"]

    assert len(stale_queries) == 2
    assert delete_by_ids_queries == []
    assert invalidation_queries == [{"path": abs_path}, {"path": abs_path}, {"path": abs_path}, {"path": abs_path}]
