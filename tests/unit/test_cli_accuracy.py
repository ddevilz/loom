...
def test_query_command_uses_vector_index_not_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr("loom.core.LoomGraph", lambda graph_name="loom", gateway=None: graph)
    monkeypatch.setattr("loom.search.searcher.FastEmbedder", _FakeEmbedder)
    
    result = runner.invoke(
        loom.cli.app,
        ["query", "authentication password validation", "--graph-name", "test", "--limit", "5"],
    )
    ...
    vector_queries = [log for log in graph.query_log if "db.idx.vector.queryNodes" in log["cypher"]]
