...
def _make_litellm_response(content: str) -> MagicMock:
    """Build a mock litellm response with the given JSON string as content."""
    msg = MagicMock()
    msg.content = content
    choice = MagicMock()
    choice.message = msg
...
@pytest.mark.asyncio
async def test_detect_violations_emits_loom_violates_report(monkeypatch) -> None:
    code = Node(
        id="function:x:f",
    ...
    monkeypatch.setattr(
        "loom.drift.detector.litellm.acompletion",
        AsyncMock(return_value=fake_response),
    )

    reports = await detect_violations([code], [doc], [edge], model="gpt-4o-mini")
...
@pytest.mark.asyncio
async def test_detect_violations_ignores_non_object_json_response(monkeypatch) -> None:
    code = Node(
        id="function:x:f",
        kind=NodeKind.FUNCTION,
        source=NodeSource.CODE,
