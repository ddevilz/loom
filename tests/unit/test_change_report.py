...
from loom.indexer.incremental import IncrementalSync
...
def test_unchanged_when_mtime_matches(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
...
def test_mtime_only_when_content_unchanged(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
    ...
    new_mtime_s = f.stat().st_mtime + 1.0
    os.utime(str(f), (new_mtime_s, new_mtime_s))

    new_mtime_ns = f.stat().st_mtime_ns
...
def test_changed_when_mtime_and_sha_differ(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
    ...
    old_mtime_ns = f.stat().st_mtime_ns
    ...
    assert f.stat().st_mtime_ns != old_mtime_ns, "mtime must differ"
...
def test_deleted_when_file_not_in_discovered(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
    """File in fingerprint store but absent from discovered_files appears in report.deleted."""
...
def test_files_to_index_is_new_plus_changed(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
    ...
    mtime_ns = f_unchanged.stat().st_mtime_ns
    ...
    mtime_m = f_mtime.stat().st_mtime_ns
    ...
    old_mtime_ns = f_changed.stat().st_mtime_ns
...
def test_empty_discovered_puts_all_stored_in_deleted(
    sync: IncrementalSync, repo: Repository, tmp_path: Path
) -> None:
