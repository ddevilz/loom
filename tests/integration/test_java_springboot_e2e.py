...
def test_java_springboot_app_parsing_accuracy():
    ...
    payment_methods = [n for n in all_nodes if n.kind == NodeKind.METHOD and "Payment.java" in n.path]
    ...
    cc_methods = [n for n in all_nodes if n.kind == NodeKind.METHOD and "CreditCardPayment.java" in n.path]
    ...
    processor_methods = [n for n in all_nodes if n.kind == NodeKind.METHOD and "PaymentProcessor.java" in n.path]
    ...
    controller_methods = [n for n in all_nodes if n.kind == NodeKind.METHOD and "PaymentController.java" in n.path]
    ...
    assert len(nodes_by_kind["class"]) >= 8, f"Expected >= 8 classes, found {len(nodes_by_kind.get('class', []))}"
    
    assert "interface" in nodes_by_kind, "No interfaces found"
    assert len(nodes_by_kind["interface"]) >= 2, f"Expected >= 2 interfaces, found {len(nodes_by_kind.get('interface', []))}"
    
    assert "enum" in nodes_by_kind, "No enums found"
    assert len(nodes_by_kind["enum"]) >= 2, f"Expected >= 2 enums, found {len(nodes_by_kind.get('enum', []))}"
    
    assert "method" in nodes_by_kind, "No methods found"
    assert len(nodes_by_kind["method"]) >= 20, f"Expected >= 20 methods, found {len(nodes_by_kind.get('method', []))}"
    ...
    assert "RestController" in annotations, f"@RestController annotation not found, got: {annotations}"
    assert "RequestMapping" in annotations, f"@RequestMapping annotation not found, got: {annotations}"
    
    # Test 16: Verify method annotations
    create_payment_methods = [n for n in all_nodes if n.name == "createPayment" and n.kind == NodeKind.METHOD]
    ...
    assert "abstract" in modifiers, f"Payment should be marked as abstract, got modifiers: {modifiers}"
...
def test_java_springboot_app_visual_graph():
    ...
    for node in domain_nodes:
        if node.kind in [NodeKind.CLASS, NodeKind.INTERFACE, NodeKind.ENUM]:
            icon = "🔷" if node.kind == NodeKind.CLASS else "🔶" if node.kind == NodeKind.INTERFACE else "🔸"
            ...
            graph_lines.append(f"  {icon} {node.name}{abstract_marker}{extends_marker}{impl_marker}")
