...
def test_properties_file_parsing():
    ...
    fixture_path = Path(__file__).parent.parent / "fixtures" / "java_springboot" / "src" / "main" / "resources" / "application.properties"
...
def test_html_file_parsing():
    ...
    try:
        ...
        if "template_engine" in metadata:
            assert metadata["template_engine"] in ["jinja2", "ejs", "php"], "Should detect template engine"
        
        # Should extract scripts
        if "scripts" in metadata:
            assert any("app.js" in script for script in metadata["scripts"]), "Should extract script references"
...
def test_markup_parsers_comprehensive():
    ...
    props_path = Path(__file__).parent.parent / "fixtures" / "java_springboot" / "src" / "main" / "resources" / "application.properties"
