...
def test_vue_tsx_app_parsing_accuracy():
    ...
    composable_functions = [n for n in all_nodes if n.kind == NodeKind.FUNCTION and "useTaskManager" in n.path]
    ...
    util_functions = [n for n in all_nodes if n.kind == NodeKind.FUNCTION and "taskHelpers" in n.path]
...
def test_vue_tsx_app_visual_graph():
    ...
    for node in type_nodes:
        if node.kind in [NodeKind.INTERFACE, NodeKind.ENUM, NodeKind.TYPE]:
            icon = "🔶" if node.kind == NodeKind.INTERFACE else "🔸" if node.kind == NodeKind.ENUM else "🔷"
    ...
    if composable_funcs:
        main_composable = next((n for n in composable_funcs if "useTaskManager" == n.name), None)
        if main_composable:
